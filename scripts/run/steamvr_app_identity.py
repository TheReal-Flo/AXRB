...
def manifest_data(package, name, icon, launcher, avd, activity, port=5580):
    arguments = subprocess.list2cmdline(['-NoProfile', '-ExecutionPolicy', 'Bypass',
        '-File', str(launcher.resolve()), '-Avd', avd, '-Port', str(port), '-Package', package, '-Activity', activity])
    app = {'app_key': app_key(package), 'launch_type': 'binary',
        'binary_path_windows': str(Path(os.environ.get('SystemRoot', 'C:/Windows')) /
            'System32/WindowsPowerShell/v1.0/powershell.exe'),
        'arguments': arguments, 'working_directory': str(launcher.resolve().parents[2]),
...
def main():
    ...
    if args.pid is None:
        ...
        args.manifest.write_text(json.dumps(manifest_data(args.package, args.name, args.icon,
            args.launcher, args.avd, args.activity, args.port), ensure_ascii=False, indent=2), encoding='utf-8')
    vr = SteamVR()
    try:
