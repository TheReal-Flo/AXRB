...
Run 'cmake' @('-S', $Source, '-B', $build, '-G', 'Ninja', '-Wno-deprecated', "-DCMAKE_MAKE_PROGRAM=$Sdk/cmake/3.22.1/bin/ninja.exe", "-DCMAKE_TOOLCHAIN_FILE=$Sdk/ndk/27.3.13750724/build/cmake/android.toolchain.cmake", "-DANDROID_ABI=$Abi", '-DANDROID_PLATFORM=android-29', '-DCMAKE_BUILD_TYPE=Release', '-DBUILD_API_LAYERS=OFF', '-DBUILD_TESTS=ON', '-DBUILD_LOADER=ON', '-DBUILD_CONFORMANCE_TESTS=OFF', '-DBUILD_ALL_EXTENSIONS=ON', "-DHELLOXR_DEFAULT_GRAPHICS_PLUGIN=$Graphics")
Run 'cmake' @('--build', $build, '--target', 'hello_xr', '-j', '8')
New-Item -ItemType Directory -Force "$build\package\lib\$Abi" | Out-Null
Copy-Item "$build\src\tests\hello_xr\libhello_xr.so" "$build\package\lib\$Abi" -Force
Copy-Item "$build\src\loader\libopenxr_loader.so" "$build\package\lib\$Abi" -Force
$manifest = Get-Content "$PSScriptRoot\emulator\AndroidManifest.xml" -Raw
if ($Abi -eq 'arm64-v8a') {
    $manifest = $manifest.Replace('com.axrb.helloxr.emulator', 'com.axrb.helloxr.emulator.arm64').Replace('AXRB Nvidia hello_xr', 'AXRB Nvidia ARM64 hello_xr')
}
if ($Graphics -eq 'Vulkan') {
    $manifest = $manifest.Replace('com.axrb.helloxr.emulator.arm64', 'com.axrb.helloxr.emulator.arm64.vulkan').Replace('package="com.axrb.helloxr.emulator"', 'package="com.axrb.helloxr.emulator.vulkan"')
    $manifest = $manifest.Replace('AXRB Nvidia hello_xr', 'AXRB Nvidia Vulkan hello_xr').Replace('AXRB Nvidia ARM64 hello_xr', 'AXRB Nvidia ARM64 Vulkan hello_xr')
}
$manifest | Set-Content "$build\AndroidManifest.xml" -Encoding UTF8
