...
int main()
{
    ...
    if (swapchainFormatCount > 1 && xrEnumerateSwapchainFormats(session, 1, &swapchainFormatCount, swapchainFormats.data()) != XR_ERROR_SIZE_INSUFFICIENT) return EXIT_FAILURE;
    if (xrEnumerateSwapchainFormats(session, static_cast<uint32_t>(swapchainFormats.size()), &swapchainFormatCount, swapchainFormats.data()) != XR_SUCCESS) return EXIT_FAILURE;
    const int64_t swapchainFormat = swapchainFormats[0];
    if (!swapchainFormat) return EXIT_FAILURE;
    ...
    std::array<XrSwapchain, 40> extraSwapchains{};
    for (auto& extra : extraSwapchains)
        if (xrCreateSwapchain(session, &swapchainCreateInfo, &extra) != XR_SUCCESS) return EXIT_FAILURE;
    for (auto extra : extraSwapchains) {
        uint32_t count = 0;
        if (xrEnumerateSwapchainImages(extra, 0, &count, nullptr) != XR_SUCCESS || count != 3) return EXIT_FAILURE;
        if (xrDestroySwapchain(extra) != XR_SUCCESS) return EXIT_FAILURE;
    }
    ...
    XrCompositionLayerEquirect2KHR sphere{};
    sphere.type = XR_TYPE_COMPOSITION_LAYER_EQUIRECT2_KHR;
    sphere.space = space; sphere.pose.orientation.w = 1;
    sphere.subImage = projectionViews[0].subImage;
    sphere.centralHorizontalAngle = 6.2831853f;
    sphere.upperVerticalAngle = 1.5707963f; sphere.lowerVerticalAngle = -1.5707963f;
    const void* sphereOnly[] = {&sphere};
    endInfo.layerCount = 1; endInfo.layers = sphereOnly;
    if (xrEndFrame(session, &endInfo) != XR_SUCCESS) return EXIT_FAILURE;
    XrCompositionLayerImageLayoutFB imageLayout{XR_TYPE_COMPOSITION_LAYER_IMAGE_LAYOUT_FB, nullptr, 1};
    struct UnknownLayerExtension { XrStructureType type; const void* next; };
    UnknownLayerExtension unknownLayout{static_cast<XrStructureType>(1999999999), &imageLayout};
    sphere.next = &unknownLayout;
    if (xrEndFrame(session, &endInfo) != XR_SUCCESS) return EXIT_FAILURE;
    imageLayout.flags = 2;
    if (xrEndFrame(session, &endInfo) != XR_ERROR_LAYER_INVALID) return EXIT_FAILURE;
    imageLayout.flags = 0;
    if (xrEndFrame(session, &endInfo) != XR_SUCCESS) return EXIT_FAILURE;
    sphere.next = nullptr;
    const void* sphereWithPanel[] = {&quads[0], &sphere};
    endInfo.layerCount = 2; endInfo.layers = sphereWithPanel;
    if (xrEndFrame(session, &endInfo) != XR_SUCCESS) return EXIT_FAILURE;
    sphere.radius = -1;
    if (xrEndFrame(session, &endInfo) != XR_ERROR_LAYER_INVALID) return EXIT_FAILURE;
    sphere.radius = 0;
    ...
    if (xrEndFrame(session, &endInfo) != XR_SUCCESS) return EXIT_FAILURE;
    sphere.subImage.imageArrayIndex = 99;
    if (xrEndFrame(session, &endInfo) != XR_ERROR_SWAPCHAIN_RECT_INVALID) return EXIT_FAILURE;
