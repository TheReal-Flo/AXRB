param(
    ...
    [switch]$GpuSharing,
    [string]$HostExe = "$PSScriptRoot\..\build-windows-nvidia\host-bridge\Release\axrb-host-bridge.exe"
)
$ErrorActionPreference = 'Stop'
if (!$PSBoundParameters.ContainsKey('GpuSharing')) {
    $GpuSharing = Test-Path "$PSScriptRoot\..\build-windows-gpu-layer\Release\axrb_gpu_layer.json"
}
...
try {
    ...
    } finally { $env:AXRB_CLOSE_EVENT = $previousCloseEvent }
    Start-Sleep -Milliseconds 800
    if ($bridgeProcess.HasExited) { throw 'Host failed to start. See build-windows-game/host.err.' }
    if ($steamIdentity) {
